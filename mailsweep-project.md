...
- [Day 19-20] **Paywall + auto-unlock in the add-on** — `Supabase.gs` calls the validate-user edge function with `Session.getActiveUser().getEmail()` before every delete (only on the first run of a sweep, continuations skip the check). When validate-user returns `allowed:false`, `onDeleteConfirm` swaps in `buildPaywallCard_` — a new card with a brand-blue "Buy for $3" button that opens the Gumroad checkout in a popup and reloads the add-on on close. Two paywall variants based on `status`: `used_today` ("come back tomorrow") and `trial_over` ("trial ended"). Deployed a second edge function `gumroad-webhook` (publicly callable, JWT off) that accepts Gumroad's form-urlencoded ping payload: on a clean sale it upserts `users.is_paid=true` and clears `is_blocked`; on refund/dispute/chargeback it flips `is_paid=false`. Optional `GUMROAD_SELLER_ID` env var for source verification. Secrets live in Apps Script Script Properties (`SUPABASE_VALIDATE_URL`, `SUPABASE_SECRET_KEY`, `GUMROAD_CHECKOUT_URL`) so they never enter the repo. End-to-end smoke test passes: simulated Gumroad ping → `is_paid` flips → validate-user returns `paid` → add-on allows delete. Real payments blocked only on PayPal KYC clearing (1-2 days).
...
- [Day 21 — Polish] **Sender breakdown card** — new killer feature: on the Preview card, a brand-blue "Review by sender" button runs `analyzeSenders_`, which samples up to 300 matching messages, groups them by sender domain, and renders top-10 senders as switch rows on a new breakdown card. Each row defaults ON; user toggles OFF to spare specific senders. On submit, unticked domains get appended as `-from:domain.com` exclusions to the sweep, the count is recomputed, and the confirm card opens against the reduced set. `buildQuery_` now accepts `excludeDomains` field on filters. On huge inboxes, per-sender counts are scaled estimates from the sample and marked with `~`. Senders outside the top 10 are still swept (noted in UI).
- [Speed pass] **Gmail Batch endpoint + UserCache for sender breakdown** — first cut of `analyzeSenders_` did one `Messages.get` per message (~50ms × 300 = blew the action wall). Rewrote to use Gmail's multipart Batch endpoint (`POST /batch/gmail/v1`): up to 100 message-get sub-requests inside one HTTP call. 300 messages = 3 round trips ≈ ~2s (vs. 15-25s timing out). New helper `gmailBatchGetFrom_` builds the multipart body, parses the multipart response, and returns From-header strings in input order. Added 5-min `CacheService.getUserCache()` keyed by an MD5 of the filter set — repeat clicks on the same filter return instantly with zero API calls.
- [Speed pass] **resultSizeEstimate for preview count** — replaced the paginate-to-10k loop in `countMatchingThreads_` (5-10s on huge inboxes) with a single `Messages.list` call that returns Gmail's index estimate alongside the first page. If results fit in one page → exact count; else → use `resultSizeEstimate` lower-bounded by what we saw, mark as `estimated`. Preview card now shows `~12,000 matching emails — Estimate from Gmail index — exact count after sweep`. Preview opens in ~500ms instead of 5-10s. Removed `COUNT_HARD_CAP` (no longer paginating).
- [Comp accounts] **Whitelist bypass for paid features** — `WHITELIST_EMAILS` array at top of `Supabase.gs`. `validateCurrentUser_` short-circuits to `{allowed:true, status:'paid'}` for whitelisted addresses (case-insensitive), skipping Supabase entirely. Used for founder + testers.
- [Day 22-23 — Launch infra] **Privacy + Terms + landing site live on GitHub Pages** at `https://rebootcode2024.github.io/mailsweep/` with the broom-on-blue logo wired in across header + favicon + OG image. OAuth consent screen finalized in GCP (Branding fields filled, 5 scopes declared with gmail.modify as restricted). CASA exemption letter drafted in `casa-exemption-letter.md` arguing MailSweep never transmits Gmail data to third-party servers (all API calls happen inside Apps Script on Google's own infrastructure; Supabase only sees the user's email for license validation). Verification submit is gated on demo video — pending recording.
- [Day 24 — Screenshots] **7 launch screenshots** under `screenshots/` covering the full flow: cluttered inbox → filter card → preview count → sender breakdown → confirm → result → save-recurring. Renamed to ordered launch sequence. Decision: ship as-is despite "5 emails" reveal-card being weak (test inbox limitation, not retake-worthy).
- [Recurring sweeps v1] **Saved filter recipes that auto-run on cadence** — paid-only feature, `src/Recurring.gs`. Initial design used one trigger per recipe; refactored to **single shared dispatcher trigger per user** after hitting Apps Script's silent trigger-create rate limit during testing. `runDueRecurringSweeps` fires daily at 3am script TZ, walks recipes, decides which are due via cadence + lastRunAt + recipe.weekday/monthday captured at create time. Save / pause / rename / cadence-change are pure UserProperties writes — trigger system untouched after bootstrap. Cap 25 recipes per user, surfaced as "X of 25" header on the home card with a contextual hint near the cap. Opt-in digest email after each run (off by default — emailing the user about Gmail cleanup defeats the inbox-zero purpose).
- [Storage cleanup] **New "Free up storage" Quick Clean section** with 3 presets targeting Gmail's 15GB quota crisis: Huge emails (>25MB), Big attachments (>10MB), Old & heavy (5y+ 5MB). `buildQuery_` extended with `sizeMinMB` / `hasAttachment` / `olderThan` operators. `countMatchingThreads_` samples sizeEstimate from up to 50 messages via the Gmail Batch endpoint and scales to total count, returning `bytesEstimate` so preview shows "~340 MB can be freed". Confirm + result cards add storage-aware copy + an "Open Gmail Trash" button so users can empty Trash to free space immediately (we deliberately don't permanent-delete on their behalf — staying inside `gmail.modify`).
- [Mark-as-read] **Second verb across the existing sweep flow.** New "Clear unread badge" Quick Clean section with 4 presets (old unread / unread Promotions / unread Social / unread Updates). Each preset sets `filters.action='read'`, which threads through preview → confirm → result with mark-as-read copy, blue primary button, and a different icon. Backend `markMatchingRead_` mirrors `deleteMatchingThreads_` but calls `batchModify` with `removeLabelIds:['UNREAD']`. `buildQuery_` gained an `extraQuery` raw-passthrough so presets inject `is:unread category:promotions` without adding a struct field for every Gmail operator. **No new scopes.** Recurring sweeps stay trash-only for v1 (action field on recipe schema is a v1.1 task).
- [Pricing] **Locked at $5 USD lifetime** after considering $20 and ₹499/month subscription. $5 matches the set-and-forget product nature, eliminates subscription support burden, lets us launch fast. Per-sale net to Indian bank: ~$3.41/₹285 after Gumroad (10% + $0.50) + PayPal (2.9% + $0.30) + USD→INR FX (~4%). CASA cost (only relevant past 100 active users) ranges $5k-$15k initial (US labs) down to ~₹50k/yr via TAC Security (Indian lab); we have a strong architectural exemption case.
